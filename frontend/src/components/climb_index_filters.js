...
const MapFilters = ({ onApply, initialSelectedGrades = [], initialSelectedAreas = [], grades, areas = [] }) => {
  ...
  // Separate Boulder and Lead grades
  const boulderGrades = grades.filter(grade => grade.startsWith('V'));
  const leadGrades = grades.filter(grade => !grade.startsWith('V'));
